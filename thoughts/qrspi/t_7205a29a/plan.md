...
- [ ] Write the file per spec §"Backend test architecture":
...
- [ ] xfail(strict) markers ONLY on: import-preview, transaction-create,
...
Verify:
- [ ] `cd backend && python -m pytest tests/test_contract_seams.py -v`
...
- [ ] Write the file per spec §"Frontend test architecture": fs-read api.ts +
      api-contracts.json, parse callers, bidirectional coverage asserts,
      mirrored suppression guard.
Verify:
- [ ] `cd frontend && npx vitest run src/tests/contract.test.ts` green;
      full `npx vitest run` 6 files / 25+ tests green.

## Phase 3 — suppression guard proof + merge gate

- [ ] Prove the guard in BOTH directions (temp moves, restored afterwards):
...
- [ ] `git add -A && git commit` (backend+frontend test files + thoughts/).
- [ ] Run ~/.hermes/fleet/merge-gate.py --repo /home/hermes/finance-tracker --no-seams
      → report verdict JSON.
